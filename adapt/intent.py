...
from .log import log_parse_tag
_log = getLogger("Adapt Intent")
DEBUG_INTENTS = (
    'mycroft-weather.mycroftai:handle_current_weather',
)
DEBUG_SKILL = 'mycroft-weather.mycroftai'
...
class Intent(object):
    ...
    def log_validate_with_tags(self, tags, confidence):
        """Validate whether tags has required entites for this intent to fire
        ...
        _log.info("*****************************************")
        _log.info("***      ADAPT INTENT MATCHING           ")
        _log.info("***")
        _log.info("*** INTENT NAME: " + self.name)
        for require_type, attribute_name in self.requires:
            required_tag, canonical_form, tag_confidence = \
                find_first_tag(local_tags, require_type)
            if not required_tag:
            ...
            #if self.name.startswith(DEBUG_SKILL):
            # if self.name in DEBUG_INTENTS:
            _log.info("***")
            _log.info("*** REQUIRED ATTRIBUTE: " + str(attribute_name))
            _log.info("*** \tTYPE: " + str(require_type))
            _log.info("*** \tTAG: ")
            log_parse_tag(_log, required_tag)
            _log.info("*** \tCANONICAL FORM: " + str(canonical_form))
            _log.info("*** \tTAG CONFIDENCE: " + str(tag_confidence))
            _log.info("*** \tNEW INTENT CONFIDENCE LEVEL: " + str(intent_confidence))
        ...
        if len(self.at_least_one) > 0:
            ...
            if not best_resolution:
                ...
                if best_resolution in local_tags:
                    local_tags.remove(best_resolution[key][0])
                _log.info("***")
                _log.info("*** AT LEAST ONE: " + str(self.at_least_one))
                _log.info("*** \tBEST RESOLUTION: ")
                for key, tags in best_resolution.items():
                    _log.info(f"*** \t\t{key}" )
                    for tag in tags:
                        log_parse_tag(_log, tag)
                _log.info("*** \tNEW INTENT CONFIDENCE LEVEL: " + str(intent_confidence))
        ...
        for optional_type, attribute_name in self.optional:
            ...
            intent_confidence += tag_confidence
            _log.info("***")
            _log.info("*** OPTIONAL ATTRIBUTE: " + str(attribute_name))
            _log.info("*** \tTYPE: " + str(optional_type))
            _log.info("*** \tTAG: ")
            log_parse_tag(_log, optional_tag)
            _log.info("*** \tCANONICAL FORM: " + str(canonical_form))
            _log.info("*** \tTAG CONFIDENCE: " + str(tag_confidence))
            _log.info("*** \tNEW INTENT CONFIDENCE LEVEL: " +  str(intent_confidence))
        ...
        total_confidence = (intent_confidence / len(tags) * confidence) \
            if tags else 0.0
        _log.info("***")
        _log.info('*** CONFIDENCE: ')
        _log.info('*** \tINTENT CONFIDENCE: ' + str(intent_confidence))
        _log.info('*** \tNUMBER OF TAGS: ' + str(len(tags)))
        _log.info('*** \tINTENT ARG: ' + str(confidence))
        _log.info("*** \tTOTAL CONFIDENCE: " + str(total_confidence))
        _log.info("*****************************************")
